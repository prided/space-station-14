### Gravity Generator


## UI

gravity-generator-window-title = Generador de gravedad

## UI field names

gravity-generator-window-status = Estado:
gravity-generator-window-power = Comidas:
gravity-generator-window-eta = PD:
gravity-generator-window-charge = Carga:

## UI statuses

gravity-generator-window-status-fully-charged = Totalmente cargado
gravity-generator-window-status-off = Fuera
gravity-generator-window-status-charging = Carga
gravity-generator-window-status-discharging = Descargas

## UI Power Buttons

gravity-generator-window-power-on = ON
gravity-generator-window-power-off = FUERA
gravity-generator-window-power-label = { $draw } / { $max } W

## UI ETA label

gravity-generator-window-eta-none = N/A
gravity-generator-window-eta-value = { TOSTRING($left, "m\\:ss") }

gravity-generator-unanchoring-failed = No se puede desanclar un generador de gravedad activo.
