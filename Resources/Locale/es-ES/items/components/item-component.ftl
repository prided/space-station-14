## PickUpVerb

pick-up-verb-get-data-text = Recoger

# "pick up" doesn't make sense if the item is already in their inventory

pick-up-verb-get-data-text-inventory = Tómalo en la mano
item-component-on-examine-size = Tiene tamaño [bold]{ $size }[/bold].
item-component-size-Tiny = diminuto
item-component-size-Small = pequeño
item-component-size-Normal = mediano
item-component-size-Large = grande
item-component-size-Huge = enorme
item-component-size-Ginormous = gigantesco

item-component-size-Colossal = colosal
